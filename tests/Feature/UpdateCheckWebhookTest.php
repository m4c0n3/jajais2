<?php

namespace Tests\Feature;

use App\Models\WebhookEndpoint;
use Firebase\JWT\JWT;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\File;
use Tests\TestCase;

class UpdateCheckWebhookTest extends TestCase
{
    use RefreshDatabase;

    private string $privateKey = <<<'KEY'
-----BEGIN PRIVATE KEY-----

Oxc17jlHz/rrU1etqleQcpg1
-----END PRIVATE KEY-----
KEY;

    private string $publicKey = <<<'KEY'
-----BEGIN PUBLIC KEY-----

IQIDAQAB
-----END PUBLIC KEY-----
KEY;

    public function test_update_check_dispatches_webhook_delivery(): void
    {
        config([
            'updates.jwt_public_key' => $this->publicKey,
            'updates.jwt_issuer' => 'updates.test',
            'updates.jwt_audience' => 'jajais',
            'updates.channel' => 'stable',
        ]);

        $token = JWT::encode([
            'iss' => 'updates.test',
            'aud' => 'jajais',
            'exp' => time() + 3600,
            'updates' => [
                [
                    'id' => 'module-helloworld-1.1.0',
                    'type' => 'module',
                    'module_id' => 'HelloWorld',
                    'version' => '1.1.0',
                    'channel' => 'stable',
                    'download_url' => 'https://updates.test/helloworld.zip',
                ],
            ],
        ], $this->privateKey, 'RS256');

        $manifestPath = storage_path('app/updates/test-manifest.jwt');
        File::ensureDirectoryExists(dirname($manifestPath));
        File::put($manifestPath, $token);

        WebhookEndpoint::create([
            'name' => 'Update sink',
            'url' => 'https://webhooks.test/update',
            'is_active' => true,
            'secret' => 'secret',
            'events' => ['update.available'],
            'headers' => null,
            'timeout_seconds' => 10,
            'max_attempts' => 3,
            'backoff_seconds' => [1, 2, 5],
        ]);

        $this->artisan('update:check --manifest='.$manifestPath)->assertExitCode(0);

        $this->assertDatabaseHas('webhook_deliveries', [
            'event' => 'update.available',
        ]);
    }
}
